...
# Fix Railway DATABASE_URL ANTES do Pydantic processar
_raw_db = os.environ.get("DATABASE_URL", "")
if _raw_db.startswith("postgres://"):
    os.environ["DATABASE_URL"] = _raw_db.replace("postgres://", "postgresql+asyncpg://", 1)
elif _raw_db.startswith("postgresql://") and "+asyncpg" not in _raw_db:
    os.environ["DATABASE_URL"] = _raw_db.replace("postgresql://", "postgresql+asyncpg://", 1)
...
class BusinessConfig:
    ...
    def endereco(self) -> str:
        return self._raw.get("mercadinho", {}).get("endereco", "Conjunto Chácara do Cabula, 74 Box 09, Salvador - BA")
    ...
    def msg_fora_horario(self) -> str:
        return self._raw.get("horario", {}).get("msg_fora_horario", "Estamos fechados agora! Voltamos amanhã às 7h 😊")
    ...
    def msg_fora_horario_delivery(self) -> str:
        return self._raw.get("delivery", {}).get("msg_fora_horario_delivery", "Delivery disponível de segunda a sexta, das 8h às 20h 😊")
    ...
    def saudacao(self) -> str:
        return self._raw.get("personalidade", {}).get("saudacao", "Olá! Bem-vindo ao FN Mercadinho 👋")
    ...
    def quando_nao_entende(self) -> str:
        return self._raw.get("personalidade", {}).get("quando_nao_entende", "Desculpa, não entendi 😅 Pode repetir?")

    @property
    def quando_sem_estoque(self) -> str:
        return self._raw.get("personalidade", {}).get("quando_sem_estoque", "Poxa, tá em falta 😕 Posso sugerir outra coisa?")

    @property
    def quando_fora_area(self) -> str:
        return self._raw.get("personalidade", {}).get("quando_fora_area", "Só entregamos no condomínio, mas pode buscar! 😊")
